...
class EurorackPmod(Elaboratable):
    ...
    def elaborate(self, platform) -> Module:
        ...
        clk_fs_sync = Signal()
        m.submodules.strobe_ff = FFSynchronizer(
                i=clk_fs, o=clk_fs_sync, o_domain="sync")
        m.submodules.fs_edge = fs_edge = EdgeToPulse()
        m.d.comb += [
            fs_edge.edge_in.eq(clk_fs_sync),
            self.fs_strobe.eq(fs_edge.pulse_out),
        ]
        # TODO: also sync cal outs? bring this to USB codebase?
